...
library(CellChat)
genes<-intersect(rownames(seu.mtc@assays$RNA) ,
...
table(seu.mtc$type)
meta.data.mtc<-seu.mtc@meta.data[,c("sample","orig.ident","title","type","MTC_cluster")]
...
samples<-intersect(colnames(xcounts),rownames(meta.data))
setdiff(rownames(meta.data),samples)
setdiff(samples,colnames(xcounts))
...
meta.data$type<-ifelse(meta.data$type%in%c('LUAD','Lung'),'Lung','TNBC')
...
cc.obj <- createCellChat(object = seu, 
                         group.by = "ident", 
                         assay = "RNA")
#cc.obj@meta$type<-ifelse(cc.obj@meta$type%in%c('LUAD','Lung'),'Lung','TNBC')
...
net$source[net$source%in%c('EMT_high','EMT_high_hypoxia', 
                           'EMT_low','Rep_high')]<-'Tumor'
sources.use = c('Tumor')
levels(cc.obj@idents)[levels(cc.obj@idents)%in%c('EMT_high','EMT_high_hypoxia', 
                                                 'EMT_low','Rep_high')]<-'Tumor'
...
targets.use = c('DAM','DAM-like.Mac','EMT_high','EMT_high_hypoxia', 
                'EMT_low','Rep_high'); 
sources.use = c('DAM','DAM-like.Mac','APOE.Mac','EMT_high','EMT_high_hypoxia', 
                'EMT_low','Rep_high')
df.net <- subsetCommunication(cc.obj, 
                              slot.name = "net",
...
df.net$source[df.net$source%in%c('EMT_high','EMT_high_hypoxia', 
                                 'EMT_low','Rep_high')]<-'Tumor'
df.net$target[df.net$target%in%c('EMT_high','EMT_high_hypoxia', 
                                 'EMT_low','Rep_high')]<-'Tumor'
...
qs::qsave(df.net,'./Lung/05cellchat/new/TAM_MTC_d6.qs')
...
table(df.net$target)
source.target <- paste(rep(sources.use, each = length(targets.use)), targets.use, sep = " -> ")
df.net$target <- factor(df.net$target, levels = c('CD8.T','Treg','DC'))
...
ggplot(df.net, aes(x = source.target, y = interaction_name_2,
  ...
  theme(legend.title = element_text(size = 8), 
        legend.text = element_text(size = 6))
qs::qsave(df.net,'./Lung/05cellchat/new/TAM_T_d6.qs')
