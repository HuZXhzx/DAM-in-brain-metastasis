...
library(scCustomize);library(scRNAtoolVis);library(clusterProfiler)
...
saveRDS(mc.mat, "./dataset2Scenic/input/dataset2.luad.brca.mat.rds")
## (1) TF list文件
motif2tfs <- data.table::fread("./dataset2Scenic/motifs_v10nr_clust_nr.hgnc_m0.001_o0.0.tbl")
TFs <- sort(unique(motif2tfs$gene_name))
writeLines(TFs, "./dataset2Scenic/cisTarget_db_hsa_hgnc_tfs.txt")

## (2) meta cell matrix (for step1): *.csv or *.loom
mc.mat <- readRDS("./dataset2Scenic/input/dataset2.luad.brca.mat.rds")
...
dir.create("./dataset2Scenic/input")
loom <- SCopeLoomR::build_loom(
  file.name         = "./dataset2Scenic/input/dataset2_mat_for_step1.loom",
  dgem              = mc.mat,
...
## inputs
f_loom_grn=input/dataset2_mat_for_step1.loom
## outputs
...
regulons <- clusterProfiler::read.gmt("./dataset2Scenic/output/Dataset2BRME.regulons.gmt")
...
saveRDS(regulon.list, "dataset2Scenic/output/regulons.rds")
...
VlnPlot(seu.TAM, group.by = "celltype_pl", features = "pcaRAS_1", split.by = "type",
        pt.size = 0, split.plot = TRUE, sort = F, cols = c("blue", "red")) + ylab("TF activity")
qs::qsave(seu.TAM, "./dataset2Scenic/seu.TAM_addScenic.qs")
...
to.label <- subset(vd.res, celltype_pl > 0.4 & type > 0.10)
...
to.label <- subset(data.use, DAM>0.7)
...
data.table::fwrite(rssMat,
                   './dataset2Scenic/seu.TAM_rssMat.txt',sep='\t',
                   quote = F,row.names = F)
data.table::fwrite(data.use,
                   './dataset2Scenic/TAM.celltype_regulon.txt',sep='\t',
                   quote = F,row.names = F)

## Key gene regulon Figure4G----
seu.TAM<-qs::qread("./dataset2Scenic/seu.TAM_addScenic.qs")
genes<-readRDS('../DAM/KEY_Gene_DAM_ALL.rds')
regulons <- read.gmt("./dataset2Scenic/output/Dataset2BRME.regulons.gmt")
e.regulon <- enricher(gene = genes, TERM2GENE = regulons, minGSSize = 10, maxGSSize = 5000)

pdf('./dataset2Scenic/KEY_Gene_DAM_regulon_dataset2.pdf',height=4,width=6)
dotplot(e.regulon,showCategory = 50)
dev.off()
saveRDS(e.regulon@result,'./dataset2Scenic/KEY_Gene_DAM_regulon_dataset2.rds')

genes<-readRDS('../DAM/Key_Gene_DAM_MDM_ALL.rds')
regulons <- read.gmt("./dataset2Scenic/output/Dataset2BRME.regulons.gmt")
e.regulon <- enricher(gene = genes, TERM2GENE = regulons, minGSSize = 10, maxGSSize = 5000)

pdf('./dataset2Scenic/KEY_Gene_DAM-like.Mac_regulon_dataset2.pdf',height=4,width=6)
dotplot(e.regulon,showCategory = 50)
dev.off()
saveRDS(e.regulon@result,'./dataset2Scenic/KEY_Gene_DAM-like.Mac_regulon_dataset2.rds')
...
e.regulon <- enricher(gene = genes, TERM2GENE = regulons, minGSSize = 10, maxGSSize = 5000)
saveRDS(e.regulon@result,file='./dataset2Scenic/TNF_TFs_d2.rds')
...
data<-data.table::fread('./dataset2Scenic/TAM.celltype_regulon.txt')
data<-data%>% 
  arrange(desc(DAM)) %>% 
