...
seu.mtc<-qs::qread('./Lung/05cellchat/04TumorCluster/seu.mtc.qs')
...
net$source[net$source%in%c('EMT_high','EMT_int',
                           'EMT_low','EMT_low_Rep_high',
                           'EMT_low_Rep_int')]<-'Tumor'
sources.use = c('Tumor')
levels(cc.obj@idents)[levels(cc.obj@idents)%in%c('EMT_high','EMT_int',
                                                 'EMT_low','EMT_low_Rep_high',
                                                 'EMT_low_Rep_int')]<-'Tumor'
...
net$target[net$target%in%c('EMT_high','EMT_int',
                           'EMT_low','EMT_low_Rep_high',
                           'EMT_low_Rep_int')]<-'Tumor'
targets.use = c('Tumor')
levels(cc.obj@idents)[levels(cc.obj@idents)%in%c('EMT_high','EMT_int',
...
targets.use = c('DAM','DAM-like.Mac','EMT_high','EMT_int',
                'EMT_low','EMT_low_Rep_high',
                'EMT_low_Rep_int'); 
sources.use = c('DAM','DAM-like.Mac','APOE.Mac',
                'EMT_high','EMT_int',
...
df.net$source[df.net$source%in%c('EMT_high','EMT_int',
                                 'EMT_low','EMT_low_Rep_high',
                                 'EMT_low_Rep_int')]<-'Tumor'
df.net$target[df.net$target%in%c('EMT_high','EMT_int',
                                 'EMT_low','EMT_low_Rep_high',
                                 'EMT_low_Rep_int')]<-'Tumor'
...
ggplot(df.net[df.net$target=='Tumor',], 
  ...
  scale_colour_gradientn(colors = colorRampPalette(c('#0099CC','#CC3333'))(100),
                         na.value = "white",
                         limits=c(quantile(df.net[df.net$target=='Tumor',]$prob, 0,na.rm= T),
                                  quantile(df.net[df.net$target=='Tumor',]$prob, 1,na.rm= T)),
                         breaks = c(quantile(df.net[df.net$target=='Tumor',]$prob, 0,na.rm= T),
                                    quantile(df.net[df.net$target=='Tumor',]$prob, 1,na.rm= T)),
                         labels = c("min","max")) +
...
qs::qsave(df.net,'./Lung/05cellchat/new/TAM_MTC_d2.qs')


##Cellchat between T cells and TAMs FigureS7======
rm(list=ls())
seu.TAM<-readRDS('./Figuresubmit/seu.TAM.rds')
seu.T<-qs::qread('./Lung/05cellchat/01Tcluster/seu.T.qs')
library(CellChat)
CellChatDB <- CellChatDB.human
dplyr::glimpse(CellChatDB$interaction)
CellChatDB.use <- CellChatDB 
...
meta.data.T<-seu.T@meta.data[,c("sample","orig.ident","title","group","type","celltype")]
meta.data.TAM<-seu.TAM@meta.data[,c("sample","orig.ident","title","group","type","celltype_pl")]
colnames(meta.data.T)[6]<-c('cluster')
colnames(meta.data.TAM)[6]<-c('cluster')
...
sources.use = c('DAM','DAM-like.Mac'); 
#'DAM','DAM-like.Mac','APOE.Mac',
targets.use = c('CD4.T','CD8.T.ex','CD4.T.cm','CD8.T','Treg','DC')
table(cc.obj@idents)
...
df<-df %>% 
  arrange(desc(prob),interaction_name,source,target) 
df$target[df$target=='CD8.T.ex']<-'CD8.T'
...
table(cc.obj@idents)
levels(cc.obj@idents)[levels(cc.obj@idents)=='CD8.T.ex']<-'CD8.T'
...
df.net$target<-as.character(df.net$target)
df.net$target[df.net$target=='CD8.T.ex']<-'CD8.T'
...
ggplot(df.net, aes(x = source.target, y = interaction_name_2,
  ...
  theme(legend.title = element_text(size = 8), 
        legend.text = element_text(size = 6))
qs::qsave(df.net,'./Lung/05cellchat/new/TAM_T_d2.qs')
