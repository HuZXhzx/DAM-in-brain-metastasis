...
seu.TAM<-readRDS('./dataset2/Lung/04clusterMDM/seu.TAM.rds')
...
seu.MDM <- RunPCA(seu.MDM)
seu.MDM <- RunHarmony(seu.MDM, group.by.vars = "title", reduction.use = "pca", dims.use = 1:50)

seu.MDM <- RunUMAP(seu.MDM, reduction = "harmony", dims = 1:30)
DimPlot(seu.MDM, reduction = "umap", group.by = "title")
seu.MDM <- RunTSNE(seu.MDM, reduction = "harmony", dims = 1:30)

seu.MDM <- RunPCA(seu.MDM, npcs = 100) 
seu.MDM <- RunHarmony(seu.MDM, group.by.vars = c("title"), reduction.use = "pca", dims.use = 1:100)
set.seed(13) 
seu.MDM  <- RunFDG(seu.MDM , reduction = "harmony", dims = 1:100)

(DimPlot(seu.MDM , reduction = "fr", group.by = "celltype.new", label = T,split.by = 'type') +NoLegend() + ggtitle('Celltype_Fr'))
...
names(color.MDM)<-c('APOE.Mac','DAM-like.Mac')
...
root.cells <- "GSM5645891_CTCAGAAAGGATGGAA-1"
terminal.cells <- c(
  "APOE.MDM.DAM"="GSM5645892_CTGTTTATCTATCCTA-1"
)
...
data.path.APOE.MDM.DAM <- get_path(p.curve.APOE.MDM.DAM, df=3)
data.arrow.APOE.MDM.DAM <- get_arrow(data.path.APOE.MDM.DAM, reverse = T)
...
Idents(seu.MDM)<-factor(seu.MDM$MDMtype,levels = c('APOE.Mac','DAM-like.Mac'))
