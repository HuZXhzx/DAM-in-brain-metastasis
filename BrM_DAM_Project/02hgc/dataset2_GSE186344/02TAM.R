...
library(scCustomize)
## Regular-----
seu<-qs::qread('./dataset2/Lung/03addTNBC/seu.all.lung.breast.qs')
table(seu$type)
seu.TAM<-subset(seu,celltype_cl%in%c('TAM'))
...
table(seu.TAM$type,seu.TAM$geo_accession)
seu.TAM <- RunHarmony(seu.TAM ,
                      reduction = "pca",
                      group.by.vars = "geo_accession",
                      reduction.save = "harmony")
ElbowPlot(seu.TAM, reduction = "pca", ndims = 30)
...
dir.create('./dataset2/Lung/02TAM/')
pdf('./dataset2/Lung/02TAM/seu.TAM_umap_resolution.pdf',width=18,height=18)
DimPlot(seu.TAM, reduction = "umap", group.by = paste0("RNA_snn_res.", resolution), ncol = 3, label = T)
dev.off()
seu.TAM$seurat_clusters <- as.character(seu.TAM$RNA_snn_res.0.5)
seu.TAM$TAM_cluster<-paste0('TAM.',seu.TAM$seurat_clusters)
Idents(seu.TAM)<-seu.TAM$TAM_cluster
table(seu.TAM$TAM_cluster)
Idents(seu.TAM)<-factor(seu.TAM$TAM_cluster,levels=paste0('TAM.',0:9))
seu.TAM$TAM_cluster[seu.TAM$TAM_cluster=='TAM.0']<-'TAM.2'
seu.TAM$TAM_cluster[seu.TAM$TAM_cluster=='TAM.9']<-'TAM.5'
Idents(seu.TAM)<-factor(seu.TAM$TAM_cluster,levels=paste0('TAM.',1:8))
...
Idents(seu)<-factor(Idents(seu),levels = c(paste0('TAM.',1:8),'others'))
...
data.table::fwrite(TAM_markers,
                   './dataset2/Lung/03addTNBC/BRCA_LUNG_TAM_cluster_markers.txt',sep='\t',
                   quote = F,row.names = F)
data.table::fwrite(TAM_markers2,
                   './dataset2/Lung/03addTNBC/BRCA_LUNG_TAM_cluster_markers_all.txt',sep='\t',
                   quote = F,row.names = F)
d <- c(
  #"TAM.0"="Microglia",
  "TAM.1"="S100A8.MDM", #&CD14 monocytes
  "TAM.2" = "Microglia", #Tnbc c0,c1, Luminalc1, Lungc1
  "TAM.3" = "Microglia", #luminalc2 No DAM
  "TAM.4" = "S100A8.MDM", #stressed MT1G MT1X
  "TAM.5"="DC", #luminalc6 lungc4
  "TAM.6"="APOE.MDM.DAM",
  "TAM.7"="APOE.MDM", 
  "TAM.8" = "recycling.MDM" 
  #"TAM.9" = "DC"
...
celltype_pl<-d<-c('DAM.Microglia'='DAM',
                  ...
                  'CD14 Monocytes_S100A8.MDM'='CD14.Mono',
                  'DC'='DC',
                  'recycling.MDM'='Prolif.Mac',
                  'S100A8.MDM'='S100A8.Mac')

table(seu.TAM$celltype.new)
...
library(scales)
show_col(color.TAM_hgc)

names(color.TAM_hgc)[3]<-'CD14.Mono&S100A8.Mac'
...
## Findmarkers & Enrichment TAM.celltype-------
DefaultAssay(seu.TAM)<-'RNA'
...
data.table::fwrite(er.df_TAM,
                   './Lung/04clusterMDM/TAMcelltype_enrichment.txt',sep='\t',
                   quote = F,row.names = F)
...
Idents(seu.TAM)<-factor(seu.TAM$celltype_pl,levels=c('DAM','MG','CD14.Mono','S100A8.Mac','APOE.Mac','DAM-like.Mac',
                                                     'Prolif.Mac','DC'))
DefaultAssay(seu.TAM)<-'MG_BMDM'
...
## Ucell TAM features [Katrina T et.al]------
MG_dataset<-qs::qread('./Geneset/MG_dataset_forhuman.qs')
...
data.plot$id<-factor(data.plot$id, 
                     levels = rev(c('DAM','MG','CD14.Mono','S100A8.Mac','APOE.Mac','DAM-like.Mac',
                                    'Prolif.Mac','DC')))
data.plot.wide<-data.plot[,c(1,4,5)] %>% 
  pivot_wider(names_from = 'id',values_from = 'avg.exp.scaled') %>% 
  column_to_rownames('gene')
saveRDS(data.plot.wide,'./Figuresubmit/DAM.wide.dataset2.rds')
...
saveRDS(p,'./Figuresubmit/DAM.wide.dataset2_v2.rds')
...
p <- t(AverageExp) %>% scale() %>% t()
saveRDS(p,'./Figuresubmit/TAM.features.wide.dataset2.rds')

##TAM-features plot Figure3C------
celltype_pl<-d<-c('DAM.Microglia'='DAM',
                  'Microglia'='MG',
                  'APOE.MDM'='APOE.Mac',
                  'APOE.MDM.DAM'='DAM-like.Mac',
...
##DAM cellmarkers Hotmap Figure4E----
...
data.plot.wide<-data.plot[,c(1,4,5)] %>% 
  pivot_wider(names_from = 'id',values_from = 'avg.exp.scaled')
saveRDS(data.plot.wide,'./Figuresubmit/data.plot.wide.dataset2.rds')
saveRDS(data.plot.wide,'./Figuresubmit/data.plot.wide.dataset2_2.rds')
...
##DAM-like.Mac cellmarkers Supplementary FigureS5A------
...
##save--------
qs::qsave(seu.TAM,'./dataset2/Lung/03addTNBC/seu.TAM.lung.breast.qs')
