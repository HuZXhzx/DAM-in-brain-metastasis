...
library(Nebulosa)
library(scCustomize)
seu<-qs::qread('./dataset1/Lung/03addBreast/seu.all.lung.breast.qs')
seu.TAM<-subset(seu,celltype=='TAM')
## seu.TAM regular-----
DimPlot(seu.TAM,split.by = 'title')
...
dir.create('./dataset1/Lung/02TAM/')
pdf('./dataset1/Lung/02TAM/seu.TAM_umap_resolution.pdf',width=18,height=18)
...
dev.off()
seu.TAM$seurat_clusters <- as.character(seu.TAM$RNA_snn_res.1.2)
...
data.table::fwrite(TAM_markers,
                   './dataset1/Lung/02TAM/TAM_TAM_cluster_markers.txt',sep='\t',
                   quote = F,row.names = F)

TAM_markers$gene[as.character(TAM_markers$cluster)=='2'] %>% head(40)
...
Idents(seu)<-factor(Idents(seu),levels = c(paste0('c',1:7),'others'))
...
d <- c(
  "TAM.0"="APOE.MDM.DAM", #Macro_MT1H SPP1
  "TAM.1"="APOE.MDM",#SPP1
  "TAM.2" = "DAM.Microglia", 
  "TAM.3" = "APOE.MDM",
  "TAM.4" = "APOE.MDM.DAM",#Macro_MT1H CXCL9
  "TAM.5"="Differenting.MDM",#Stressed
  "TAM.6"="CD14.Monocytes",
  "TAM.7"="S100A8.MDM", #Macro_PPARG
  "TAM.8" = "DC", 
  "TAM.9" = "Differenting.MDM",#Macro_LYVE1
  "TAM.10" = "Recycling.MDM" #APOE.MDM
)
##Dimplot Figure3A------
...
color.TAM_hgc<-c("#B294C7",#DAM
                 "#E8D3E2",#MG
                 "#D8AC60",#CD14 monocyets
                 "#B49D99",#APOE.MDM
                 "#98D277",#APOE.MDM.DAM
                 "#F2E4C4", #Differenting.MDM
                 "#8EC694", #Recycling.MDM
                 "#F9E8D7", #S100A8.MDM
                 "#C48244",#DC
                 "#F6955A",#Neu
                 "#E9D2D0", #Mac.M1TH
                 "#6B4C4A" ,#Stressed.Mac
                 "#6D9BC3" #'Lung.derived.Mac'
) 
names(color.TAM_hgc)<-c('DAM','MG','CD14.Mono',
                        'APOE.Mac','DAM-like.Mac',
                        'Diff.Mac','Prolif.Mac','S100A8.Mac','DC','Neu',
                        'MT1H.Mac','Stressed.Mac','Lung.derived.Mac')
saveRDS(color.TAM_hgc,'../DAM/color.TAM_hgc.rds')
...
pdf('./dataset1/Lung/02TAM/MG_BMDM_avg.pdf',width=6,height=4)
...
pdf('./dataset1/Lung/02TAM/TAMfeatures_avg.pdf',width=5,height=3)
Idents(seu.TAM)<-factor(Idents(seu.TAM),levels = paste0('c',1:6))
AverageHeatmap(object = seu.TAM ,
               assays = "TAM.features",
...
table(seu.TAM$TAM_cluster,seu.TAM$title)
qs::qsave(seu.TAM,file='./dataset1/Lung/02TAM/seu.TAM.lung.qs')
...
Idents(seu.TAM)<-seu.TAM$TAM_cluster
seu.TAM@meta.data %>% rownames()
pdf('./dataset1/Lung/02TAM/TAM_brain_DAM.pdf',width=6,height=3)
Idents(seu.TAM)<-factor(Idents(seu.TAM),levels = paste0('c',1:6))
AverageHeatmap(object = seu.TAM,
               assays = "DAM",
...
pdf('./Figuresubmit/MG_BMDM_avg.Recluster.pdf',height=2.5,width=4)
Idents(seu.TAM)<-factor(seu.TAM$celltype_pl,levels=c('DAM','CD14.Mono','S100A8.Mac','APOE.Mac','DAM-like.Mac',
                                                     'Prolif.Mac',
                                                     'Stressed.Mac',
                                                     'DC'))
DefaultAssay(seu.TAM)<-'MG_BMDM'
...
AverageHeatmap(object = seu.TAM ,
               assays = 'MG_BMDM',height=2,width=6,
               markerGene =c('Core-MG','Core-BMDM','TAM-MG','TAM-BMDM'),
               clusterAnnoName = F,
...
data.plot$id<-factor(data.plot$id, 
                     levels = rev(c('DAM','CD14.Mono','S100A8.Mac','APOE.Mac','DAM-like.Mac',
                                    'Prolif.Mac',
                                    'Stressed.Mac',
                                    'DC')))
...
dev.off()
##TAM celltype markers and Enrichment-----
DefaultAssay(seu.TAM)<-'RNA'
...
saveRDS(data.plot.wide,'./Figuresubmit/data.plot.wide.dataset1_2.rds')
