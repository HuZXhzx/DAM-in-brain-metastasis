...
library(UCell);library(clusterProfiler);library(harmony);library(CellChat)

##Cellchat between tumor and TAMs------
seu.TAM<-qs::qread('./06cellchat/01ccTcell/seu.TAM.brain.celltype.new.qs')
seu.tumor<-qs::qread('./06cellchat/03ccbrain_Tumor/seu.brain.tumor.qs')
...
meta.data.mtc<-seu.tumor@meta.data[,c("sample","orig.ident","title","resource","MTC_cluster")]
meta.data.TAM<-seu.TAM@meta.data[,c("sample","orig.ident","title","resource","celltype_pl")]
colnames(meta.data.mtc)[5]<-c('cluster')
...
dir.create('./06cellchat/new/')
qs::qsave(cc.obj,'./06cellchat/new/cc.obj_mtc.new.qs')
data.table::fwrite(df.net,
                   './06cellchat/new/df_net_DAM_MTC.new.txt',sep='\t',
                   quote = F,row.names = F)

##Chord plot Figure7B  Tumor->TAMs----
rm(list=ls())
cc.obj<-qs::qread('./06cellchat/new/cc.obj_mtc.new.qs')
...
sources.use = c('EMT_high',
                'EMT_low')
net <- subset(net, source %in% sources.use)
net <- subset(net, target %in% targets.use)
net$source<-as.character(net$source)
net$source[net$source%in%c('EMT_high',
                           'EMT_low')]<-'Tumor'
sources.use = c('Tumor')
levels(cc.obj@idents)[levels(cc.obj@idents)%in%c('EMT_high',
                                                 'EMT_low')]<-'Tumor'
...
net$target[net$target%in%c(
  'EMT_high',
  'EMT_low')]<-'Tumor'
targets.use = c('Tumor')
levels(cc.obj@idents)[levels(cc.obj@idents)%in%c(
  'EMT_high',
  'EMT_low')]<-'Tumor'
...
targets.use = c('DAM','DAM-like.Mac','EMT_high',
                'EMT_low'); 
sources.use = c('DAM','DAM-like.Mac','APOE.Mac','EMT_high',
                'EMT_low')
df.net <- subsetCommunication(cc.obj, 
                              slot.name = "net",
...
df.net$source[df.net$source%in%c('EMT_high', 
                                 'EMT_low')]<-'Tumor'
df.net$target<-as.character(df.net$target)
df.net$target[df.net$target%in%c('EMT_high', 
                                 'EMT_low')]<-'Tumor'
...
qs::qsave(df.net,'./06cellchat/new/TAM_MTC_L1.qs')


##Cellchat between T cells and TAMs FigureS7=====
seu.TAM<-readRDS('./FigureSubmit/seu.TAM.brain.rds')
seu.T<-qs::qread('./06cellchat/01ccTcell/seu.T.qs')
seu.T<-subset(seu.T,resource=='brain')
genes<-intersect(rownames(seu.T@assays$RNA) ,
                 rownames(seu.TAM@assays$RNA))
xcounts<-cbind(seu.T@assays$RNA@counts[genes,],
               seu.TAM@assays$RNA@counts[genes,])
table(seu.TAM$celltype)
table(seu.T$celltype)
meta.data.T<-seu.T@meta.data[,c("sample","orig.ident","title","resource","celltype")]
meta.data.TAM<-seu.TAM@meta.data[,c("sample","orig.ident","title","resource","celltype_pl")]
...
qs::qsave(cc.obj,'./06cellchat/new/cc.obj.T.new.qs')
data.table::fwrite(df.net,
                   './06cellchat/new/df.net_brain_solo_Tcell.txt',sep='\t',
                   quote = F,row.names = F)
...
DAM_T_pathway<-c('LGALS9 - CD45',
                 ...
                 'SPP1 - CD44',
                 'CD99 - CD99')
cc.obj<-qs::qread('./06cellchat/new/cc.obj.T.new.qs')
table(cc.obj@idents)
...
sources.use = c('DAM','DAM-like.Mac'); 
#'DAM','DAM-like.Mac','APOE.Mac',
targets.use = c('CD8.T.ex','CD4.T.cm','CD8.T.em','Treg','DC')
table(cc.obj@idents)
...
targets.use = c('CD4.T','CD8.T','CD8.T.ex','Treg','DC')
...
df.net$target[df.net$target=='CD4.T.cm']<-'CD4.T'
df.net$source.target <- paste(df.net$source, 
                              df.net$target, sep = " -> ")
df.net$id<-paste(df.net$source.target,df.net$interaction_name_2)
df.net<-df.net %>% 
...
ggplot(df.net, aes(x = source.target, y = interaction_name_2,
  ...
  theme(legend.title = element_text(size = 8), 
        legend.text = element_text(size = 6))
qs::qsave(df.net,'./06cellchat/new/TAM_T_L1.qs')
