...
(DimPlot(seu.TAM, reduction = "umap",group.by='TAM_cluster', label = T)+NoLegend())+
  (DimPlot(seu.TAM, reduction = "umap",group.by='celltype')) & ggsci::scale_color_d3("category20")
...
qs::qsave(seu.MDM, "./07trajectory/Brain.seu.MDM.trajectory.qs")
DefaultAssay(seu.MDM)<-'RNA'
seu <- seu.MDM[rownames(seu.MDM[["RNA"]]@scale.data), ]
sceasy::convertFormat(seu, from = "seurat", to = "anndata", main_layer = "scale.data", 
                      outFile =  "./07trajectory/Brain.seu.MDM.scaled.h5ad")

seu.MDM<-qs::qread( "./07trajectory/Brain.seu.MDM.trajectory.qs")
...
palantir.res<-data.table::fread('./07trajectory/palantir_results.csv') 
...
pdf('./07trajectory/MDM.pseudotime.pdf',height=4,width=8)
...
root.cells <- "GSM3516671-131217354767276"
terminal.cells <- c(
  "APOE.MDM.DAM" = "GSM3516671-241114589555955")
