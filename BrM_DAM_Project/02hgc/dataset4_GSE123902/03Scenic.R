...
table(seu$resource)
seu.list <- SplitObject(seu, split.by = "resource")
seu.list <- lapply(seu.list, function(object) {
  object@project.name <- unique(object$resource)
  return(object)
})
...
dir.create('./05Scenic/input')
saveRDS(mc.mat, "./05Scenic/input/Lung1.mat.rds")
## (1) TF list文件
...
writeLines(TFs, "./05Scenic/cisTarget_db_hsa_hgnc_tfs.txt")

## (2) meta cell matrix (for step1): *.csv or *.loom
mc.mat <- readRDS("./05Scenic/input/Lung1.mat.rds")
...
loom <- SCopeLoomR::build_loom(
  file.name         = "./05Scenic/input/Lung1_mat_for_step1.loom",
  dgem              = mc.mat,
...
## inputs
f_loom_grn=./input/Lung1_mat_for_step1.loom
## outputs
...
##AUCell-----
regulons <- clusterProfiler::read.gmt("./05Scenic/output/Lung1BRME.regulons.gmt")
...
saveRDS(regulon.list, "05Scenic/output/regulons.rds")
...
qs::qsave(seu.TAM, "./05Scenic/seu.TAM_addScenic.qs")
...
data.table::fwrite(rssMat,
                   './05Scenic/seu.TAM.brain_rssMat.txt',sep='\t',
                   quote = F,row.names = F)
data.table::fwrite(data.use,
                   './05Scenic/TAM.celltype_regulon.txt',sep='\t',
                   quote = F,row.names = F)

## Key gene regulon Figure4G----
seu.TAM<-qs::qread("./05Scenic/seu.TAM_addScenic.qs")
genes<-readRDS('~/Brain/brian/DAM/KEY_Gene_DAM_ALL.rds')
regulons <- read.gmt("./05Scenic/output/Lung1BRME.regulons.gmt")
e.regulon <- enricher(gene = genes, TERM2GENE = regulons, minGSSize = 10, maxGSSize = 5000)

pdf('./05Scenic/KEY_Gene_DAM_regulon_Lung1.pdf',height=4,width=6)
dotplot(e.regulon,showCategory = 50)
dev.off()
saveRDS(e.regulon@result,'./05Scenic/KEY_Gene_DAM_regulon_Lung1.rds')

genes<-readRDS('~/Brain/brian/DAM/Key_Gene_DAM_MDM_ALL.rds')
regulons <- read.gmt("./05Scenic/output/Lung1BRME.regulons.gmt")
e.regulon <- enricher(gene = genes, TERM2GENE = regulons, minGSSize = 10, maxGSSize = 5000)

pdf('./05Scenic/KEY_Gene_DAM-like.Mac_regulon_Lung1.pdf',height=4,width=6)
dotplot(e.regulon,showCategory = 50)
dev.off()
saveRDS(e.regulon@result,'./05Scenic/KEY_Gene_DAM-like.Mac_regulon_Lung1.rds')
...
e.regulon <- enricher(gene = genes, TERM2GENE = regulons, minGSSize = 10, maxGSSize = 5000)
saveRDS(e.regulon@result,file='./05Scenic/TNF_TFs_l1.rds')
...
data<-data.table::fread('./05Scenic/TAM.celltype_regulon.txt')
data<-data%>% 
  arrange(desc(DAM)) %>% 
