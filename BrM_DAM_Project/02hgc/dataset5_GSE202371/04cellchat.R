...
seu.tumor<-qs::qread('./03cellchat/02ccTumor/seu.tumor.qs')
genes<-intersect(rownames(seu.tumor@assays$RNA) ,
                 rownames(seu.TAM@assays$RNA))
table(seu.TAM$celltype)
xcounts<-cbind(seu.tumor@assays$RNA@counts[genes,],
               seu.TAM@assays$RNA@counts[genes,])
meta.data.tumor<-seu.tumor@meta.data[,c("sample","orig.ident","title","MTC_cluster")]
meta.data.TAM<-seu.TAM@meta.data[,c("sample","orig.ident","title","celltype_pl")]
colnames(meta.data.tumor)[4]<-c('cluster')
colnames(meta.data.TAM)[4]<-c('cluster')
meta.data<-rbind(meta.data.tumor,meta.data.TAM)
...
table(cc.obj@idents)
dir.create('./03cellchat//new/')
qs::qsave(cc.obj,'./03cellchat/new/cc.obj_mtc.new.qs')
data.table::fwrite(df.net,
                   './03cellchat/new/df_net_DAM_MTC.new.txt',sep='\t',
                   quote = F,row.names = F)
netVisual_bubble(cc.obj,
                 targets.use = c('DAM','DAM-like.Mac'), 
                 sources.use = c('DAM','DAM-like.Mac','APOE.Mac',
                                 'EMT_high_REP_high',"EMT_low",
                                 'EMT_high_REP_low','EMT_low_p53_low','EMT_low_REP_high'), 
                 remove.isolate = FALSE,signaling = c("ApoE","APP"))
netVisual_chord_gene(cc.obj,
                     targets.use = c('DAM'), 
                     sources.use =  c('DAM','DAM-like.Mac','APOE.Mac',
                                      'EMT_high_REP_high',"EMT_low",
                                      'EMT_high_REP_low','EMT_low_p53_low','EMT_low_REP_high'),
                     lab.cex = 0.5,legend.pos.y = 30,signaling = c("ApoE","APP"))

##Chord plot Figure7B  Tumor->TAMs----
rm(list=ls())
cc.obj<-qs::qread('./03cellchat/new/cc.obj_mtc.new.qs')
...
#   pairLR.use.i <- searchPair(signaling = signaling[i], pairLR.use = cc.obj@LR$LRsig, key = "pathway_name", matching.exact = T, pair.only = T)
...
table(net$source)
targets.use = c('DAM','DAM-like.Mac'); 
sources.use = c('EMT_high_REP_high',"EMT_low",
                'EMT_high_REP_low','EMT_low_p53_low','EMT_low_REP_high')
net <- subset(net, source %in% sources.use)
net <- subset(net, target %in% targets.use)
net$source<-as.character(net$source)
levels(cc.obj@idents)[levels(cc.obj@idents)%in%sources.use]<-'Tumor'

net$source[net$source%in%sources.use]<-'Tumor'
sources.use = c('Tumor')
...
color.use <- c("Tumor"="#6A3D9A",
               color.TAM_hgc[c('DAM','DAM-like.Mac','APOE.Mac')])
...
pdf('./Figuresubmit/TAM&MTC.pdf',height=10,width=10)  
...
targets.use = c('EMT_high_REP_high',"EMT_low",
                'EMT_high_REP_low','EMT_low_p53_low','EMT_low_REP_high')
net <- subset(net, source %in% sources.use)
net <- subset(net, target %in% targets.use)
net$target<-as.character(net$target)
net$target[net$target%in%c('EMT_high_REP_high',"EMT_low",
                           'EMT_high_REP_low','EMT_low_p53_low','EMT_low_REP_high')]<-'Tumor'
targets.use = c('Tumor')
levels(cc.obj@idents)[levels(cc.obj@idents)%in%c('EMT_high_REP_high',"EMT_low",
                                                 'EMT_high_REP_low','EMT_low_p53_low','EMT_low_REP_high')]<-'Tumor'
...
df.net$source[df.net$source%in%c('EMT_high_REP_high',"EMT_low",
                                 'EMT_high_REP_low','EMT_low_p53_low','EMT_low_REP_high')]<-'Tumor'
df.net$target<-as.character(df.net$target)
df.net$target[df.net$target%in%c('EMT_high_REP_high',"EMT_low",
                                 'EMT_high_REP_low','EMT_low_p53_low','EMT_low_REP_high')]<-'Tumor'
...
ggplot(df.net, aes(x = source.target, y = interaction_name_2,
  ...
  scale_radius(range = c(min(df.net$pval), max(df.net$pval)),
               ...
               name = "p-value")+
  scale_colour_gradientn(colors = colorRampPalette(c('#0099CC','#CC3333'))(100),
                         na.value = "white",
                         limits=c(quantile(df.net$prob, 0,na.rm= T),
                                  quantile(df.net$prob, 1,na.rm= T)),
                         breaks = c(quantile(df.net$prob, 0,na.rm= T),
                                    quantile(df.net$prob, 1,na.rm= T)),
                         labels = c("min","max")) +
...
qs::qsave(df.net,'./03cellchat/new/TAM_MTC_L2.qs')


##Cellchat between T cells and TAMs FigureS7------
seu.TAM<-readRDS('./Figuresubmit/seu.TAM.celltype_pl.rds')
seu.T<-qs::qread('./03cellchat/01ccTcell/seu.T.qs')
genes<-intersect(rownames(seu.T@assays$RNA) ,rownames(seu.TAM@assays$RNA))
matrix_TAM<-seu.TAM@assays$RNA@counts[genes,]
matrix_TAM['CD4',]<-0
xcounts<-cbind(seu.T@assays$RNA@counts[genes,],
               matrix_TAM)
table(meta.data.T$celltype)
meta.data.T<-seu.T@meta.data[,c("sample","orig.ident","title","celltype")]
meta.data.TAM<-seu.TAM@meta.data[,c("sample","orig.ident","title","celltype_pl")]
colnames(meta.data.T)[4]<-c('cluster')
colnames(meta.data.TAM)[4]<-c('cluster')
...
cc.obj<- aggregateNet(cc.obj)
df.net <- subsetCommunication(cc.obj) 
qs::qsave(cc.obj,'./03cellchat/new/cc.obj_T.new.qs')
data.table::fwrite(df.net,
                   './03cellchat/new/df_net_DAM_T.new.txt',sep='\t',
                   quote = F,row.names = F)
...
DAM_T_pathway<-c('LGALS9 - CD45',
                 ...
                 'SPP1 - CD44',
                 'CD99 - CD99')
cc.obj<-qs::qread('./03cellchat/new/cc.obj_T.new.qs')

netVisual_chord_gene(cc.obj,
                     
                     targets.use = c('CD8.T.ex','CD4.T.cm','CD8.T.em','CD8.T','Treg','DC'), 
                     sources.use = c('DAM','DAM-like.Mac'),
                     lab.cex = 0.5,
                     legend.pos.y = 30,
                     signaling = c("CD99") )
...
table(cc.obj@idents)
df.net<-data.table::fread('./03cellchat/new/df_net_DAM_T.new.txt')
prob <- slot(cc.obj, "net")$prob
pval <- slot(cc.obj, "net")$pval
prob[pval > 0.05] <- 0
net <- reshape2::melt(prob, value.name = "prob")
...
df$target[df$target%in%c('CD8.T.em')]<-'CD8.T'
df$target[df$target=='CD4.T.cm']<-'CD4.T'
table(df$target)
df$id<-paste(df$source,df$target,df$interaction_name,sep='.')
df<-df %>% 
  arrange(id,desc(prob)) %>% 
...
table(cc.obj@idents)
levels(cc.obj@idents)[levels(cc.obj@idents)%in%c('CD8.T.em')]<-'CD8.T'
levels(cc.obj@idents)[levels(cc.obj@idents)=='CD4.T.cm']<-'CD4.T'
targets.use = c('CD4.T','CD8.T.ex','CD8.T','Treg','DC')
...
color.use <- c("CD4.T"="#56B4E9","CD8.T"="#B55D8D","CD8.T.ex"="#FF69B4",
               "Treg"="#00BFC4",
...
circos.clear()
library(circlize);
pdf('./Figuresubmit/TAM&Tcell2.pdf',height=10,width=10)  
chordDiagram(df.plot,
             order = order.sector,
             col = edge.color,
             grid.col = grid.col,
...
#pairLR <- extractEnrichedLR(cc.obj, signaling = c("ApoE","APP"), geneLR.return = FALSE)
sources.use = c('DAM','DAM-like.Mac'); 
targets.use = c('CD8.T.ex','CD4.T.activate','CD4.T','CD8.T.em','CD8.T','Treg','DC')
...
df.net$target<-as.character(df.net$target)
df.net$target[df.net$target%in%c('CD8.T.ex','CD8.T.em')]<-'CD8.T'
df.net$target[df.net$target=='CD4.T.activate']<-'CD4.T'
df.net$source.target <- paste(df.net$source, 
                              df.net$target, sep = " -> ")
df.net$id<-paste(df.net$source.target,df.net$interaction_name_2)
df.net<-df.net %>% 
...
ggplot(df.net, aes(x = source.target, y = interaction_name_2,
  ...
  theme(legend.title = element_text(size = 8), 
        legend.text = element_text(size = 6))
qs::qsave(df.net,'./03cellchat/new/TAM_T_L2.qs')
