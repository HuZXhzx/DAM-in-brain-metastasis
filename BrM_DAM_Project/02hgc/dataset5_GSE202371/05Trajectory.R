...
seu.MDM<-subset(seu.MDM,TAM_cluster=='TAM.11',invert=T)
...
root.cells <- "GSM6112141-TCTCTTCAAACACATGTCTCAGATTCA"
terminal.cells <- c(
  "APOE.MDM.DAM" = "GSM6112145-TATGTGGCAACAGTGGTAAAAGTCATT"
)
...
##difference MDM cluster Figure6G-----
DefaultAssay(seu.MDM)<-'RNA'
Idents(seu.MDM)<-seu.MDM$TAM_cluster
markers_DAM.MDM<-FindMarkers(seu.MDM,ident.1 = 'TAM.3',ident.2 = 'TAM.2')
...
Key_Gene_DAM_MDM<-readRDS('~/Brain/brian/DAM/Key_Gene_DAM_MDM.rds')
FeaturePlot_scCustom(seu.MDM, reduction = "fr", features = Key_Gene_DAM_MDM)

data.table::fwrite(markers_DAM.MDM,
                   './05trajectory/DAM_vs_Orig.txt',sep='\t',
                   quote = F,row.names = F)
