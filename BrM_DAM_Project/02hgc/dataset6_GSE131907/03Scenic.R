...
seu$Smoking %>% table()
DimPlot(seu, group.by = "celltype",split.by = 'Smoking',label=T)
...
seu.list <- SplitObject(seu, split.by = "Tissue_origins")
seu.list <- lapply(seu.list, function(object) {
  object@project.name <- unique(object$Tissue_origins)
  return(object)
})
names(seu.list)<-c("primary","primary_late","Brmt")
...
saveRDS(mc.mat, "./04Scenic/input/Lung3.mat.rds")
saveRDS(mc.mat, "./04Scenic/input/Lung3.all.mat.rds")
...
mc.mat <- readRDS("./04Scenic/input/Lung3.mat.rds")
mc.mat <- readRDS("./04Scenic/input/Lung3.all.mat.rds")
...
loom <- SCopeLoomR::build_loom(
  file.name         = "./04Scenic/input/Lung3_all_mat_for_step1.loom",
  dgem              = mc.mat,
...
## inputs
f_loom_grn=./input/Lung3_all_mat_for_step1.loom
## outputs
...
regulons <- clusterProfiler::read.gmt("./04Scenic/output/Lung3BRME.regulons.gmt")
...
VlnPlot(seu.TAM, group.by = "celltype_pl", features = "pcaRAS_2", split.by = "type",
...
data.table::fwrite(rssMat,
                   './04Scenic/seu.TAM.brain_rssMat.txt',sep='\t',
                   quote = F,row.names = F)
...
regulons <- read.gmt("./04Scenic/output/Lung3BRME.regulons.gmt")
e.regulon <- enricher(gene = genes, TERM2GENE = regulons, minGSSize = 10, maxGSSize = 5000)

pdf('./04Scenic/KEY_Gene_DAM_regulon_Lung3.pdf',height=4,width=6)
dotplot(e.regulon,showCategory = 50)
dev.off()
saveRDS(e.regulon@result,'./04Scenic/KEY_Gene_DAM_regulon_Lung3.rds')

genes<-readRDS('~/Brain/brian/DAM/Key_Gene_DAM_MDM_ALL.rds')
regulons <- read.gmt("./04Scenic/output/Lung3BRME.regulons.gmt")
e.regulon <- enricher(gene = genes, TERM2GENE = regulons, minGSSize = 10, maxGSSize = 5000)

pdf('./04Scenic/KEY_Gene_DAM-like.Mac_regulon_Lung3.pdf',height=4,width=6)
dotplot(e.regulon,showCategory = 50)
dev.off()
saveRDS(e.regulon@result,'./04Scenic/KEY_Gene_DAM-like.Mac_regulon_Lung3.rds')
...
e.regulon <- enricher(gene = genes, TERM2GENE = regulons, minGSSize = 10, maxGSSize = 5000)
saveRDS(e.regulon@result,file='./04Scenic/TNF_TFs_l3.rds')
