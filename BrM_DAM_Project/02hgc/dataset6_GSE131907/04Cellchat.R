...
seu.tumor$sample<-rownames(seu.tumor@meta.data)
...
targets.use = c('EMT_high',"EMT_high_ANG_high",
  'EMT_high_REP_high','EMT_low')
net <- subset(net, source %in% sources.use)
net <- subset(net, target %in% targets.use)
net$target<-as.character(net$target)
net$target[net$target%in%c('EMT_high',"EMT_high_ANG_high",
                           'EMT_high_REP_high','EMT_low')]<-'Tumor'
targets.use = c('Tumor')
levels(cc.obj@idents)[levels(cc.obj@idents)%in%c('EMT_high',"EMT_high_ANG_high",
                                                 'EMT_high_REP_high','EMT_low')]<-'Tumor'
...
df.net$source[df.net$source%in%c('EMT_high',"EMT_high_ANG_high",
                                 'EMT_high_REP_high','EMT_low')]<-'Tumor'

df.net$target<-as.character(df.net$target)
df.net$target[df.net$target%in%c('EMT_high',"EMT_high_ANG_high",
                                 'EMT_high_REP_high','EMT_low')]<-'Tumor'
...
qs::qsave(df.net,'./03cellchat/new/TAM_MTC_L3.qs')
...
netVisual_bubble(cc.obj,
                 sources.use = c('DAM','DAM-like.Mac'), 
                 targets.use = c('CD4.T','CD4.T.activate','CD8.T','CD8.T.em','CD8.T.ex','Treg','DC'), 
...
df$target[df$target=='CD4.T.activate']<-'CD4.T'
table(df$target)
df$id<-paste(df$source,df$target,df$interaction_name,sep='.')
df<-df %>% 
  arrange(id,desc(prob)) %>% 
...
levels(cc.obj@idents)[levels(cc.obj@idents)=='CD4.T.activate']<-'CD4.T'
...
ggplot(df.net, aes(x = source.target, y = interaction_name_2,
  ...
  theme(legend.title = element_text(size = 8), 
        legend.text = element_text(size = 6))
qs::qsave(df.net,'./03cellchat/new/TAM_T_L3.qs')
